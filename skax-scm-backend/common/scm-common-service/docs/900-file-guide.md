...
| Method | 경로               | 설명             | 기타 |
|--------|------------------|----------------|----|
| POST   | /api/v1/common/files/upload          | 단건파일을 업로드 합니다. |    |
| POST   | /api/v1/common/files/upload/multiple | 다건파일을 업로드 합니다. |    |
...
| Method | 경로        | 설명            | 기타 |
|--------|-----------|---------------|----|
| POST   | /api/v1/common/files/download | 파일을 다운로드 합니다. |    |

## 2. 환경설정 
### 2.1 filePolicy-{profile}.yml 
3 Depth 부터 원하는 정책Key를 선언하여 정책 속성들을 정의하면 됩니다.
참고로, 아래 내용이 `UploadPolicies.java` 파일에 그대로 맵핑됩니다.
