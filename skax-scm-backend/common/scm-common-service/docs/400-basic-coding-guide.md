...
1. **Code Template 및 Javadoc 사용**
    - 현재 가이드는 **IDE 독립적**으로 적용합니다.
    - VSCode 기준 권장 확장: `Extension Pack for Java`, `Lombok Annotations Support for VS Code`
    - Javadoc은 IDE 플러그인 의존 없이 수동 작성 또는 코드 스니펫(템플릿)으로 관리합니다.

2. **클래스/메서드 설명**
   - 클래스(class)의 주석은 신규 파일 생성 시점에 템플릿/스니펫으로 자동 생성하거나, 수동으로 동일 포맷을 유지합니다.
   - **클래스(class) 주석은 패키지명, 파일명, 작성자, 작성일, description 을 기본으로 설정 한다.**
     ```java
     /**
      * packageName      : scm.common.biz.sample.service
      * fileName         : SampleService.java
      * author           : honggildong
      * date             : 2026-03-26
      * description      : 샘플 아이템 비즈니스 로직
      */
     ```
