...
![sw_architecture_plantuml.png](images/guide/sw_architecture_plantuml.png)
> PlantUML 원본: [sw_architecture_plantuml.puml](images/guide/sw_architecture_plantuml.puml)
...
### 2.2 **용이성**
- Infrastructure Layer 구현(MyBatis Mapper, 외부 연동)의 변경이 있더라도 상위 레이어(Service, Controller)에는 영향이 없도록 설계했습니다.
...
- **RepositoryPort Interface**: 기반기술 중 영속성 기술을 담당
    - 현재 영속성(MyBatis) 기술의 세부 구현을 추상화합니다.
...
> 도메인 객체는 영속성 기술과도 분리되어 있으므로 관계형 모델/기술 변경 시 그대로 유지됩니다.  
...
> 기반기술 계층의 경우 외부와의 통신을 담당합니다.  
> 기반기술로는 메시징,메일, 데이터 접근 등 많은 기반기술이 있으나  
> 본 프로젝트는 데이터 접근 역할을 주로 구현 하였습니다.

- **기술 선택의 유연성**:
    - 현재 Persistence 기술은 MyBatis 기반으로 구현되며 Repository Interface로 추상화됩니다.
