...
---
## **사용예제**
> `RedisContextStorageServiceTest` 및 `ContextController` 파일 참고
...
---
## 참고사항
- 컨텍스트 저장소는 인증 사용자(`SecurityContext`의 uid)가 있어야 동작합니다.
- 구현체는 **`RedisContextStorageService`** 이며, 키 접두 `scm:context:` 와 TTL(60분)으로 Redis에 저장합니다.
- `get` 호출 시 해당 사용자 컨텍스트 키의 TTL을 60분으로 갱신합니다.
